...
require_once dirname(__FILE__) . '/lib/wpapper_core.class.php';
require_once dirname(__FILE__) . "/lib/common/json_url.inc.php";
require_once dirname(__FILE__) . "/admin/wpapper-admin.class.php";
require_once dirname(__FILE__) . '/api/' . WPAPPER_API_VERSION . '/wpapper_api.php';
class mobileplugin_wpapper {
    /**
     * init
     */
    public static function init(){
        global $wp;
        $wp->add_query_var( WpApperConf::$route_prefix );
        $wp->add_query_var( WpApperConf::$app_prefix );
    ...
    public static function plugin_get_url_prefix() {
        return apply_filters( 'plugin_get_url_prefix', WpApperConf::$app_prefix );
    }
     ...
     */
    public static function wpapper_json_api_maybe_flush_rewrites() {
        $version = get_option( 'bigapp_json_api_plugin_version', null );
        if ( empty( $version ) ||  $version !== WPAPPER_APP_VERSION) {
            flush_rewrite_rules();
            update_option( 'bigapp_json_api_plugin_version', WPAPPER_APP_VERSION);
        }
    }
    /**
     * Load the JSON API.
     ...
     */
    public static function wpapper_json_api_loaded() {
        if ( empty( $GLOBALS['wp']->query_vars[WpApperConf::$app_prefix] ) )
            return;
        ...
        $wp_json_server->serve_request( $GLOBALS['wp']->query_vars[WpApperConf::$route_prefix] );
        // We're done.
        die();
    }
    ...
    public static function wpapper_json_api_default_filters($server ) {
        ...
        add_action( 'deprecated_function_run', 'wpapper_json_handle_deprecated_function', 10, 3 );
        add_filter( 'deprecated_function_trigger_error', '__return_false'                         );
        add_action( 'deprecated_argument_run', 'wpapper_json_handle_deprecated_argument', 10, 3 );
    ...
    public static function wpapper_json_handle_options_request($response, $handler ) {
    ...
    //login
    public static function wpapper_auth_login($redirect_to, $requested_redirect_to, $user){
		
		if ( !isset($_GET[WpApperConf::$app_prefix]) )
            return $redirect_to;

        $route_prefix = WpApperConf::$route_prefix;
        if(isset($_GET[$route_prefix]) && $_GET[$route_prefix] == 'logout'){
            wpapper_core::yz_auth_logout();
        }

        if(isset($_GET["action"]) && $_GET["action"] == 'register'){
            wpapper_core::yz_auth_register();
        }

        if(isset($_GET[$route_prefix]) && $_GET[$route_prefix] == 'getcaptcha'){
            wpapper_core::yz_auth_getcaptcha();
        }
		
		if(isset($_GET[$route_prefix]) && $_GET[$route_prefix] == 'sociallogin'){
            wpapper_core::yz_auth_social_login();
        }
        ...
        if ( !is_wp_error($user) ){
            ...
            $result['data'] = array(
                ...
                'avatar' => wpapper_json_get_avatar_url($user->user_email?$user->user_email:$user->ID),
                'roles' => $user->roles,
				'logout_url' => wp_create_nonce('logout'),
            );
			$result['data']['hasbind'] = wpapper_core::yz_auth_social_bind($user->ID);
        }else{
        ...
        wpapper_core::set_response($result);
    }
	
	public static function wpapper_auth_register(){
        if(isset($_GET['wpapper_app']) && $_GET['wpapper_app'] == 1){
		    wpapper_core::yz_auth_register();
        }
    }
    ...
    public static function wpapper_json_api_activation($network_wide ) {
        if ( version_compare( $GLOBALS['wp_version'], WPAPPER_MINIMUM_WP_VERSION, '<' ) ) {
            return;
        }
     ...
     */
    //禁用插件，主要是删除重写规则
    public static function wpapper_json_api_deactivation($network_wide ) {
        if ( function_exists( 'is_multisite' ) && is_multisite() && $network_wide ) {
     ...
     */
    ///home/work/webroot/wordpress/wp-fcm_settings.php:354行开始
     ...
     *                             otherwise true.
     */
    public static function wpapper_json_cookie_check_errors($result ) {
        ...
        if ( isset( $_REQUEST['_wp_json_nonce'] ) ) {
            $nonce = sanitize_text_field($_REQUEST['_wp_json_nonce']);
    ...
    public static function wpapper_json_cookie_collect_status() {
        global $wp_json_auth_cookie;
     ...
     */
    public static function wpapper_json_cookie_collect_status_valid() {
        global $wp_json_auth_cookie;
     ...
     * 检测到重复评论的时候，会进入这个接口
     */
    public static function wpapper_json_api_duplicate_comment($comment_data){
        $result = array("error_code"=>BigAppErr::$comment['code'],
            ...
            "data"=>wpapper_lan('dup comment'));
        wpapper_core::set_response($result);
    }
    /**
     * 检测到评论过快的时候,会进入该接口
     */
    public static function wpapper_json_api_flood_comment($time_lastcomment=0, $time_newcomment=0){
        $result = array("error_code"=>BigAppErr::$comment['code'],
     ...
     * 定制化头像
     */
    public static function wpapper_json_api_get_avatar($avatar, $id_or_email='', $size=array(), $default='', $alt= array(), $args=array() ){
        //判定用户是否有特定的头像
        $myavatar = wpapper_get_image_avatar_by_id_or_email($id_or_email);
        $avatar = sprintf(
            "<img  src='%s'  class='%s' height='%d' width='%d' />",
            //esc_attr( $args['alt'] ),
            esc_url( $myavatar),
            "class",
            50,50
     ...
     * 国际化,可自定义语言映射
     */
    public static function wpapper_json_api_languages($translations, $text='', $domain=''){
        if($domain == WpApperConf::$languages_prefix){
            return "琼爷";
        }
        return $translations;
    }
     ...
     * 本地化 
     */
    public static function wpapper_localize() {
        load_plugin_textdomain( WpApperConf::$languages_prefix,false, dirname(plugin_basename(__FILE__)).'/languages');
    }
}

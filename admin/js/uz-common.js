    ...
    Uz = function () {
        ...
        var setMenu = function(v,url){
                            ...
                            switch(j){
                                case 'name' :
                                    if(setConfsData[i][j] === undefined || setConfsData[i][j] === ''){
                                        error_txt = 'App menu name can not be empty'
                                    }else{
            ...
            function getConfItem(val){
                var setturl = '<td><a href="'+v.menu_conf.opt_url+'&menu_id='+val.ID+'&menu_name='+encodeURIComponent(val.name)+'" class="button">Content setting</a></td>';
        ...
        var setBanner = function(data){
            ...
            $('.major-publishing-actions h3').html(menu_name+'-Banner Setting');
            var banner_length = 0;
            var baseData = {
                "name": "",
                "ID": '',
                        ...
                        switch(i){
                            case 'name' :
                                if(d[i] === undefined || d[i] === ''){
                                    error_txt = 'The title cannot be empty!'
                                }else{
                            ...
                            case 'img_url' :
                                if(!d[i].isURL()){
                                    console.log(i);
                                    error_txt = 'The image cannot be empty. Please upload image!';
                                }else{
                                    d[i] = encodeURI(d[i]);
                                }
                                break;
                            case 'link' :
                                if(!d[i].isURL()){
                                    console.log(i);
                                    error_txt = 'Please fill out the link address in the correct link';
                                }else{
                                    d[i] = d[i];
                                }
                                break;
            ...
            function getConfItem(value){
                ...
                var str = ''+
                    ...
                    '            <button class="btn-file">Select</button>'+
                    '            <input type="file" onchange="uploadFile(\'' + imgUrl + '&key=upload_img_' + banner_length + '\' , \'img_up_'+ banner_length+'\' , \'img_url_'+ banner_length+'\' , \'File upload fail\', \'uploading...\', \'OK\');" class="input-file">'+
                    '            <input type="text" name="img_url" value="' + val.img_url + '" class="hidden" hidden="">'+
                    '            <div class="file-result">' + ( val.img_url == '' ? 'Click choose image' :'Click change image') + '</div>'+
                    '        </div>'+
                    '    </td>'+
                    '    <td>'+
                    ...
                    '    <td>'+
                    '                <select name="type">'+
                    '                    <option  value="1" ' + (val.type == 1 ? 'selected="selected"' : '') + '>External link</option>'+
                    '                    <option  value="2" ' + (val.type == 2 ? 'selected="selected"' : '') + '>Article link</option>'+
                    '                    <option  value="3" ' + (val.type == 3 ? 'selected="selected"' : '') + '>Menu link</option>'+
                    '                </select>'+
                    '    </td>'+
                    '    <td>'+
                    '        <input type="text" name="rank" value="'+(val.rank ? val.rank:0)+'">'+
        ...
        return{
            ...
            drawIndex: function(data,urlList) {
                ...
                var updateData = function (data) {
                    ...
                    $('#uz #menu_info').each(function () {
                        ...
                        if (val.menu_switch == '1') {
                            ...
                            o.find(ns + 'switch').attr('title', 'Stop the function').html('Stop');
                        } else {
                            o.removeClass('active').addClass('inactive');
                            o.find(ns + 'switch').attr('title', 'Start the function').html('Start');
                        }
                    ...
                    $('#uz #fcm_setting_info').each(function () {
                        var o = $(this);
                        var val = data.fcm_setting_info;
                        o.find(ns + 'link').attr('href', val.opt_url);

                    });

                    $('#uz #send_notification_info').each(function () {
                        var o = $(this);
                        var val = data.send_notification_info;
                        o.find(ns + 'link').attr('href', val.opt_url);

                    });

                    $('#uz #app_style').each(function () {
                        var o = $(this);
                        var val = data.list_info;
                        o.find(ns + 'link').attr('href', val.opt_url);
                    ...
                    // if ( verify.ak == '' ){
                        ...
                        // $('#submit').attr("onclick","location.href='http://wpapper.com'");
                    // }
                    $('#submit').attr("onclick","location.href='/wp-admin/admin.php?page=wpapper_admin'");
                ...
                $("#submit").click(function(){
                    ...
                    $.ajax({
                        type: 'post',
                        url:  "http://wpapper.com/?wpapper_app=1&api_route=wpapper_api&action=get_test_app",
                        dataType: 'json',
                        data: v,
                        success: function (json) {
                            console.log(json);
                            ...
                            window.location.href = "/wp-admin/admin.php?page=wpapper_admin";
                        },

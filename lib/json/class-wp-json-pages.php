...
class WP_JSON_Pages extends WP_JSON_CustomPostType {
	...
	public function register_routes( $routes ) {
		...
		$routes[ $this->route] = array(
			'get_post_by_path' => array( array( $this, 'get_post_by_path' ),    WP_JSON_Server::READABLE ),
			"edit_post_by_path" => array( array( $this, 'edit_post_by_path' ),   WP_JSON_Server::EDITABLE | WP_JSON_Server::ACCEPT_JSON ),
			"delete_post_by_path" => array( array( $this, 'delete_post_by_path' ), WP_JSON_Server::DELETABLE ),
		);
	...
	public function get_post_by_path( $path, $context = 'view' ) {
		...
		if ( empty( $post ) ) {
            wpapper_json_error(BigAppErr::$post['code'],BigAppErr::$post['msg'],"");
		}
	...
	protected function prepare_post( $post, $context = 'view', $show_type= 'row' ) {
		...
		$_post['meta']['links']['self'] = json_url( $this->base . '/' . get_page_uri( sanitize_text_field($post['ID']) ) );

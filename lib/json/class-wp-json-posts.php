...
class WP_JSON_Posts {
	...
	public function get_revisions( $id ) {
		...
		if ( empty( $id ) || empty( $parent['ID'] ) ) {
            wpapper_json_error(BigAppErr::$post['code'],"Invalid post ID.");
		}

		if ( ! wpapper_json_check_post_permission( $parent, 'edit' ) ) {
            wpapper_json_error(BigAppErr::$post['code'],__("Sorry, you cannot view the revisions for this post."));
 		}
	...
	public function get_posts( $filter = array(), $context = 'view', $type = 'post', $page = 1,$offline=false ) {
		...
		foreach ( (array) $type as $type_name ) {
			...
			if ( ! ( (bool) $post_type ) || ! $post_type->show_in_json ) {
                wpapper_json_error(BigAppErr::$post['code'],"Invalid post type");
			}
        ...
        wpapper_show_debug($query,__FILE__,__LINE__);
		//重新建立主循环，从DB中找出指定文章
		$post_query = new WP_Query();
		$posts_list = $post_query->query( $query );
        wpapper_show_debug($posts_list,__FILE__,__LINE__);
		...
		foreach ( $posts_list as $post ) {
			...
			if ( ! wpapper_json_check_post_permission( $post, 'read' ) ) {
				continue;
			}
	...
	public function create_post( $data ) {
		...
		if ( $result == false) {
            wpapper_json_error(BigAppErr::$post['code'],"create post faild!");
		}

		$response = wpapper_json_ensure_response( $this->get_post( $result, 'edit' ) );
	...
	public function get_post( $id, $context = 'view' ,$is_featured_image=false) {
        ...
        wpapper_show_debug($post,__FILE__,__LINE__);

		if ( empty( $id ) || empty( $post['ID'] ) ) {
            wpapper_json_error(BigAppErr::$post['code'],"get post faild!",$id);
		}
		...
		if ( 'inherit' === $post['post_status'] && $post['post_parent'] > 0 ) {
            ...
            wpapper_show_debug($_temp_post,__FILE__,__LINE__);
		...
		if ( ! wpapper_json_check_post_permission( $checked_post, $checked_permission ) ) {
            wpapper_json_error(BigAppErr::$post['code'],BigAppErr::$post['msg'],"cant read this post.",$id);
		}
		$response = new WP_JSON_Response();
		$data = $this->prepare_post( $post, $context );
        if($data == false){
            wpapper_json_error(BigAppErr::$post['code'],"prepare post faild.",$id);
        }
	...
	public function edit_post( $id, $data, $_headers = array() ) {
		...
		if ( empty( $id ) || empty( $post['ID'] ) ) {
            wpapper_json_error(BigAppErr::$post['code'],"post id is not valid",$id);
		}
		...
		if ( isset( $_headers['IF_UNMODIFIED_SINCE'] ) ) {
			...
			if ( $check && mysql2date( 'U', $post['post_modified_gmt'] ) > $check->format('U') ) {
                wpapper_json_error(BigAppErr::$post['code'],"There is a revision of this post that is more recent.");
			}
		}
	...
	public function delete_post( $id, $force = false ) {
		...
		if ( empty( $id ) || empty( $post['ID'] ) ) {
            wpapper_json_error(BigAppErr::$post['code'],"post id is Invalid",$id);
		}

		if ( ! wpapper_json_check_post_permission( $post, 'delete' ) ) {
            wpapper_json_error(BigAppErr::$post['code'],"Sorry, you are not allowed to delete this post.");
		}

		$result = wp_delete_post( $id, $force );

		if ( ! $result ) {
            wpapper_json_error(BigAppErr::$post['code'],"The post cannot be deleted.");
		}
		...
		if ( $force ) {
			...
			return array( 'message' => wpapper_lan( 'Deleted post' ),'id'=>$id );
		}
	}
	...
	public function get_post_type( $type, $context = 'view' ) {
		...
		if ( $type->show_in_json === false ) {
            wpapper_json_error(BigAppErr::$post['code'],"Cannot view post type");
		}
	...
	protected function prepare_post( $post, $context = 'view',$show_type= 'row' ) {
		...
		// Dates
		$timezone = wpapper_json_get_timezone();
		...
		if ( $post['post_date_gmt'] === '0000-00-00 00:00:00' ) {
			...
			$post_fields['date']              = wpapper_json_mysql_to_rfc3339( $post['post_date'] );
			$post_fields_extended['date_tz']  = $post_date->format( 'e' );
			$post_fields_extended['date_gmt'] = wpapper_json_mysql_to_rfc3339( $post['post_date_gmt'] );
		}
		...
		if ( $post['post_modified_gmt'] === '0000-00-00 00:00:00' ) {
			...
			$post_fields['modified']              = wpapper_json_mysql_to_rfc3339( $post['post_modified'] );
			$post_fields_extended['modified_tz']  = $modified_date->format( 'e' );
			$post_fields_extended['modified_gmt'] = wpapper_json_mysql_to_rfc3339( $post['post_modified_gmt'] );
		}
        ...
        if($_post['comment_status'] == 'closed'){
            ...
            $comment_type = wpapper_core::check_comment_status();
        ...
        //浏览量次数
        $post_views = new WP_JSON_PostViews($this->server);
        $_post['views'] = $post_views->get_views_by_id(sanitize_text_field($post['ID']));
	...
	protected function insert_post( array $data ) {
		...
		if ( $update ) {
			if ( ! wpapper_json_check_post_permission( $post, 'edit' ) ) {
                wpapper_json_error(BigAppErr::$post['code'],__("Sorry, you are not allowed to edit this post."));
			}

			if ( $post_type->name != get_post_type( $data['ID'] ) ) {
                wpapper_json_error(BigAppErr::$post['code'],"The post type may not be changed.");
			}
		} else {
			if ( ! wpapper_json_check_post_permission( $post, 'create' ) ) {
                wpapper_json_error(BigAppErr::$post['code'],__("Sorry, you are not allowed to post on this site."));
			}
		}
		...
		if ( ! empty( $data['status'] ) ) {
			...
			switch ( $post['post_status'] ) {
				...
				case 'pending':
					break;
				case 'private':
					if ( ! wpapper_json_check_post_permission( $post, 'publish_posts' ) ) {
                        wpapper_json_error(BigAppErr::$post['code'],__("Sorry, you are not allowed to create private posts in this post type"));
					}
					break;
				case 'publish':
				case 'future':
					if ( ! wpapper_json_check_post_permission( $post, 'publish_posts' ) ) {
                        wpapper_json_error(BigAppErr::$post['code'],__("Sorry, you are not allowed to publish posts in this post type"));
					}
					break;
				default:
		...
		// Post date
		if ( ! empty( $data['date'] ) ) {
			$date_data = wpapper_json_get_date_with_gmt( $data['date'] );

			if ( ! empty( $date_data ) ) {
				list( $post['post_date'], $post['post_date_gmt'] ) = $date_data;
			}
		} elseif ( ! empty( $data['date_gmt'] ) ) {
			$date_data = wpapper_json_get_date_with_gmt( $data['date_gmt'], true );
		...
		if ( ! empty( $data['author'] ) ) {
			...
			if ( is_object( $data['author'] ) ) {
				if ( empty( $data['author']->ID ) ) {
                    wpapper_json_error(BigAppErr::$post['code'],"Invalid author object.");
				}
			...
			if ( $data['author'] !== get_current_user_id() ) {
				if ( ! wpapper_json_check_post_permission( $post, 'edit_others_posts' ) ) {
                    wpapper_json_error(BigAppErr::$post['code'],"You are not allowed to edit posts as this user.");
				}

				$author = get_userdata( $data['author'] );

				if ( ! $author ) {
                    wpapper_json_error(BigAppErr::$post['code'],"Invalid author ID.");
				}
			}
		...
		if ( ! empty( $data['password'] ) ) {
			...
			if ( ! wpapper_json_check_post_permission( $post, 'publish_posts' ) ) {
                wpapper_json_error(BigAppErr::$post['code'],__("Sorry, you are not allowed to create password protected posts in this post type"));
			}
		}
		...
		if ( ! empty( $data['parent'] ) ) {
			...
			if ( empty( $parent ) ) {
                wpapper_json_error(BigAppErr::$post['code'],"Invalid post parent ID.");
			}
		...
		if ( ! empty( $data['post_format'] ) ) {
			...
			if ( ! in_array( $data['post_format'], $formats ) ) {
                wpapper_json_error(BigAppErr::$post['code'],"Invalid post format.");
			}
	...
	protected function check_read_permission( $post ) {
		...
		return wpapper_json_check_post_permission( $post, 'read' );
	}
	...
	protected function check_edit_permission( $post ) {
		...
		return wpapper_json_check_post_permission( $post, 'edit' );
	}
}

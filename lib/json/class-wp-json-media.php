...
class WP_JSON_Media extends WP_JSON_Posts {
	 ...
	 */
	public function get_posts( $filter = array(), $context = 'view', $type = 'attachment', $page = 1, $offline=false ) {
		if ( $type !== 'attachment' ) {
	...
	public function get_post( $id, $context = 'view', $is_featured_image=false ) {
		...
		if ( empty( $id ) ) {
            wpapper_json_error(BigAppErr::$post['code'],BigAppErr::$comment['msg'],wpapper_lan("Invalid post id"));
		}

		$post = get_post( $id, ARRAY_A );

		if ( $post['post_type'] !== 'attachment' ) {
            wpapper_json_error(BigAppErr::$post['code'],BigAppErr::$comment['msg'],wpapper_lan("Invalid post type"));
		}
	 ...
	 * @return array
	 */
	protected function prepare_post( $post, $context = 'single', $show_type= 'row' ) {

...
class WP_JSON_Taxonomies {
    ...
    public function get_nav_list($context='view'){
        $switch = get_option(WpApperConf::$option_menu_switch,0);
        $menu_list = array();
        if($switch == 1){
            $menu_list  = wpapper_sort_by_key(json_decode(get_option(WpApperConf::$option_menu_conf,"{}"),true),'rank');
            $banner_confs = json_decode(get_option(WpApperConf::$option_banner_conf),true);
            foreach($menu_list as &$list){
                if($list['type'] == 'custom' && preg_match("@".get_bloginfo('siteurl')."@i",$list['link'])){     //对于内部的文章地址
                    ...
                    $list['link'] = wpapper_convert_link2json($list['link'],2);
                    $list['type'] = 'post_type';
                }
    ...
    public function get_banner_info($id,$banner_confs){
        ...
        $lists = wpapper_sort_by_key($lists,'rank');
        foreach($lists as &$list){  //如果有一些特定链接，在这边做转换
            $list['link_org'] = $list['link'];
            $list['link'] = wpapper_convert_link2json($list['link'],$list['type']);
        }
        return $lists;
    }
	...
	protected function prepare_taxonomy_object( $taxonomy, $context = 'view' ) {
		if ( $taxonomy->public === false ) {
            wpapper_json_error(BigAppErr::$taxonomy['code'],BigAppErr::$post['msg'],wpapper_lan("Cannot view taxonomy"));
		}
	...
	public function get_post_bigapp_special($filter= array(),$page=1){
		...
		foreach($ret as &$r){
			...
			$r['link'] = json_url('wpapper_api','get_special_post_list',array('id'=>$r['ID']));
			if(!isset($r['date'])){
				$r['date'] = date("Y-m-d H:i:s");
			}
			unset($r['description']);
		}
		$ret = wpapper_sort_by_key($ret,'order');
		if(isset($page) && isset($filter['pre_page'])){
			$ret = wpapper_get_list_pagination($ret,$page,$filter['pre_page']);
		}
		return $ret;
    }
	...
	public function get_taxonomy_terms( $taxonomy, $filter = array() ) {
		if ( ! taxonomy_exists( $taxonomy ) ) {
            wpapper_json_error(BigAppErr::$taxonomy['code'],"Invalid taxonomy ID.");
		}
		...
		if ( is_wp_error( $terms ) ) {
            wpapper_json_error(BigAppErr::$taxonomy['code'],"get_term return error");
		}
	...
	public function get_taxonomy_term( $taxonomy, $term, $context = 'view' ) {
		if ( ! taxonomy_exists( $taxonomy ) ) {
            wpapper_json_error(BigAppErr::$taxonomy['code'],BigAppErr::$taxonomy['msg'],"Invalid taxonomy ID.");
		}

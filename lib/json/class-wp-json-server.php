...
class WP_JSON_Server implements WP_JSON_ResponseHandler {
	...
	public function serve_request( $path = null ) {
		...
		if ( ! $served ) {
			...
			if ( $json_error_message ) {
                wpapper_json_error(BigAppErr::$server['code'],"$json_error_message");
			}
	...
	public function dispatch() {
		switch ( $this->method ) {
			...
			default:
                wpapper_json_error(BigAppErr::$server['code'],BigAppErr::$server['msg'],"Unsupported request method.");
		}

        $route_action = isset($this->params['GET'][WpApperConf::$action_prefix])?$this->params['GET'][WpApperConf::$action_prefix]:(isset($this->params['post'][WpApperConf::$action_prefix])?$this->params['post'][WpApperConf::$action_prefix]:"index");
		foreach ( $this->get_routes() as $route => $handlers ) {
			...
			foreach ( $handlers as $action => $handler ) {
				...
				if ( ! is_callable( $callback ) ) {
                    wpapper_json_error(BigAppErr::$server['code'],BigAppErr::$server['msg'],"The handler for the route is invalid.");
				}
				...
				if ( $supported & self::ACCEPT_JSON ) {
					...
					if ( $json_error_message ) {
						...
						if ( empty( $data ) ) {
                            wpapper_json_error(BigAppErr::$server['code'],BigAppErr::$server['msg'],$json_error_message);
						}
					}
        ...
        wpapper_json_error(BigAppErr::$server['code'],BigAppErr::$server['msg'],wpapper_lan('No route was found matching the URL and request method'));
	}
	...
	protected function get_json_last_error( ) {
        $last_error_msg = wpapper_json_last_error_msg();
	...
	protected function sort_callback_params( $callback, $provided ) {
		...
		foreach ( $wanted as $param ) {
			if ( isset( $provided[ $param->getName() ] ) ) {
                ...
                wpapper_json_error(BigAppErr::$server['code'],BigAppErr::$server['msg'],wpapper_lan('Missing parameter:%s.',$param->getName()));
			}
		}
	...
	public function parse_date( $date, $force_utc = false ) {
		...
		return wpapper_json_parse_date( $date, $force_utc );
	}
	...
	public function get_date_with_gmt( $date, $force_utc = false ) {
		...
		return wpapper_json_get_date_with_gmt( $date, $force_utc );
	}
	...
	*/
	public function get_avatar_url( $email ) {
		...
		return wpapper_json_get_avatar_url( $email );
	}
	...
	public function get_timezone() {
		...
		return wpapper_json_get_timezone();
	}

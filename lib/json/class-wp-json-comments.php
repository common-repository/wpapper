...
class WP_JSON_Comments {
    ...
    public function add_comment($id,$comment,$author='',$email='',$parent_id = 0,$type=0){
		if ( empty( $id) || empty($comment) ) {
            wpapper_json_error(BigAppErr::$comment['code'],"empty id or comment");
		}
        $user_id = get_current_user_id();
        $comment_type = wpapper_core::check_comment_status();
        if($comment_type == 2 && $user_id == 0){
            if($author == '' or $email == ''){
                wpapper_json_error(BigAppErr::$comment['code'],'need email or author');
            }
            if(false == wpapper_check_email($email)){
                wpapper_json_error(BigAppErr::$comment['code'],'email format is wrong');
            }
        }
        if($comment_type == 3){
            if($user_id  == 0){
		...
		if ( ! $result ) {
            wpapper_json_error(BigAppErr::$comment['code'],"creat new comment failed");
		}
	...
	public function delete_comment( $comment, $force = false ) {
		...
		if ( empty( $comment ) ) {
            wpapper_json_error(BigAppErr::$comment['code'],BigAppErr::$comment['msg'],wpapper_lan("comment id invalid"));
		}
        ...
        if($user_id == 0){      //未登录用户,不能删除评论
            wpapper_json_error(BigAppErr::$login['code'],BigAppErr::$login['msg'],wpapper_lan("need login"));
        }

		if ( ! current_user_can(  'edit_comment', $comment_array['comment_ID'] )  && $user_id != $comment_array['user_id'] ) {
            wpapper_json_error(BigAppErr::$comment['code'],BigAppErr::$comment['msg'],wpapper_lan("no auth to delete comment "));
		}

		$result = wp_delete_comment( $comment_array['comment_ID'], $force );

		if ( ! $result ) {
            wpapper_json_error(BigAppErr::$comment['code'],BigAppErr::$comment['msg'],wpapper_lan("delete comment failed"));
		}
	...
	public function get_comments( $id,$filter=array(),$page=1) {
		...
		if ( empty( $post['ID'] ) ) {
            wpapper_json_error(BigAppErr::$comment['code'],"post id is empty",$id);
		}

        wpapper_show_debug($post,__FILE__,__LINE__);
		if ( ! wpapper_json_check_post_permission( $post, 'read' ) ) {
            wpapper_json_error(BigAppErr::$comment['code'],"cannot read this post.");
		}
        ...
        wpapper_show_debug($comments,__FILE__,__LINE__);
	...
	public function get_comment( $comment ) {
        ...
        wpapper_show_debug($comment_data,__FILE__,__LINE__);

		if ( empty( $comment ) ) {
            wpapper_json_error(BigAppErr::$comment['code'],"cannot get comment.",$comment);
		}
	...
	protected function prepare_comment( $comment, $requested_fields = array( 'comment', 'meta' ), $context = 'single' ) {
		...
		if ( (int) $comment->user_id !== 0 ) {  //d登录用户.使用用户信息中的邮箱查找头像
			...
			$fields['author'] = array(
				...
				'avatar' => wpapper_json_get_avatar_url( $comment->comment_author_email ),
			);
		}
		...
		$fields['date_gmt'] = wpapper_json_mysql_to_rfc3339( $comment->comment_date_gmt );
    ...
    public function my_comments($filter=array(),$page=10){
        ...
        $user_id = get_current_user_id();
        wpapper_show_debug("my_comments:user_id".$user_id,__FILE__,__LINE__);
        if($user_id == 0){      //not login return empty
            ...
            wpapper_show_debug($post_ids,__FILE__,__LINE__);
            if($post_ids){

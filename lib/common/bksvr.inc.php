<?php
require_once(dirname(__FILE__) . "/wpapperjson.class.php");
class BkSvr
{
	protected $_ak = null;
	protected $_sk = null;
	...
	public function getInfo($url, $param, $checkRes = true)
	{
		...
		$tmp = wpapper_addUrlQueryString($url, $param);
	...
	public function getInfoByPost($url, $param, $checkRes = true)
	{
		...
		$check['sign'] = $sign;
		$tmp = wpapper_addUrlQueryString($url, $check);
	...
	public function curlInfo($url, $postData = null) {
        ...
        curl_close($ch);
		return WPAPPERJSON::decode($data);
	}

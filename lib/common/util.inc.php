 ...
 */
function wpapper_json_handle_deprecated_function($function, $replacement, $version ) {
 ...
 * @param string $version     Version.
 */
function wpapper_json_handle_deprecated_argument($function, $message, $version ) {
    if ( ! empty( $message ) ) {
...
if ( ! function_exists('wpapper_json_last_error_msg') ):
/**
 ...
 */
function wpapper_json_last_error_msg() {
    ...
    wpapper_show_debug($last_error_code,__FILE__,__LINE__);
 ...
 */
function wpapper_json_parse_date($date, $force_utc = false ) {
	if ( $force_utc ) {
 ...
 *                    null on failure.
 */
function wpapper_json_get_date_with_gmt($date, $force_utc = false ) {
	$date = wpapper_json_parse_date( $date, $force_utc );
 ...
 * @return mixed
 */
function wpapper_json_mysql_to_rfc3339($date_string ) {
...
*/
function wpapper_json_get_avatar_url($email ) {
 ...
 */
function wpapper_json_get_timezone() {
	static $zone = null;
 ...
 *                                            instance otherwise.
 */
function wpapper_json_ensure_response($response ) {
 ...
 */
function wpapper_json_check_post_permission($post, $capability = 'read' ) {
	$permission = false;
	...
	switch ( $capability ) {
		case 'read' :
			if ( ! $post_type->show_in_json ) {
                wpapper_show_debug("post_type not show_in_json:post_id:".$post['ID']." post_type:".$post['post_type'],__FILE__,__LINE__);
				return false;
			}
			...
			if ( 'inherit' === $post['post_status'] && $post['post_parent'] > 0 ) {
                ...
                wpapper_show_debug($parent,__FILE__,__LINE__);

				if ( wpapper_json_check_post_permission( $parent, 'read' ) ) {
					$permission = true;
				}
			}
            ...
            wpapper_show_debug("post_permisson:".$permission." post_id:".$post['ID']." post_type:".$post['post_type']." post_status:".$post['post_status'],__FILE__,__LINE__);
			break;
 ...
 */
function wpapper_json_error($code, $msg='', $data=''){
    $result = array('error_code'=>$code,
            'error_msg'=>wpapper_lan($msg),
            'data'=>$data);
    $req = WpApper_Common::getInstance();
    $req->setResponse($result);
}
/**
 * 根据id/email 检测是否有自定义头像
 */
function wpapper_get_image_avatar_by_id_or_email($id_or_email){
    $upload_dir = wp_upload_dir();
    $img_file_prix = $upload_dir['basedir'].'/';
    $myavatar_prix = $upload_dir['basedir'].'/';
    $mymd5 = wpapper_sign64($id_or_email);
    if ( ! file_exists($img_file_prix.$mymd5.'.jpg')){
        $mymd5 = intval($mymd5) % 20;
        $myavatar = get_avatar_url($id_or_email);
    } else {
 ...
 * 国际化语言包的封装
 */
function wpapper_lan($text, $param=null){
    $_text = __($text,WpApperConf::$languages_prefix);
    if($param == null){
        return $_text;
    }else{
 ...
 * path 模板的路径
 */
function wpapper_get_html($file, $data, $path=''){
    if ($path == ''){
        $path = WPAPPER_ROOT."/admin/views";
    }
...
function wpapper_loadTemplate($tpl, $vars, $tplVars = null, $path = '') {
	$json = json_encode($vars);
	$js_script = '<script type="text/javascript"> _bigapp_obj = eval(\'(' . $json . ")');</script>\n";
	
	if ($path == ''){
        $path = WPAPPER_ROOT."/admin/views/".$tpl;
    }
 ...
 * 对输出的文件做编码
 */
function wpapper_conver_string_to_output($content){
...
function wpapper_echo_output($content){
    echo wpapper_conver_string_to_output($content);
}
/**
 * 按照特定的key排序多维数组
 */
function wpapper_sort_by_key($lists, $row, $sort=SORT_ASC){
 ...
 * 获取图片上传路径
 */
function wpapper_get_image_upload_path(){
    $upload_dir = wp_upload_dir();
 ...
 * 优点：用db存储。最终可以和媒体打通
 */
function wpapper_upload_img_by_wp($key='file', $image_size_type = 'banner', $prefix='banner'){
    $ret = array( 'status'=>1,
                'msg' =>'',);
	$name = $_FILES[$file_id]['name'];
    $overrides = array();
    $post_id = 0;
    $ret = wpapper_check_img($img_size_type,$_FILES[$key]);
    if($ret['status'] == 0){
 ...
 * 图片上传
 */
function wpapper_upload_img($key='upload_img', $img_size_type='banner', $prefix='banner'){
    $ret = array(
        'status'=>1,
        'msg' =>'',);
    $path = '';
    $upload_path = wpapper_get_image_upload_path();
    if(!$upload_path){
...
function wpapper_check_img($image_size_type, $file){
    $ret = array( 'status'=>1,
                  'msg' =>'',);
    ...
    $size_info = WpApperConf::$img_size[$image_size];
    $fileSize = $file['size'];
 ...
 * 发起curl请求
 */
function wpapper_curl_info($url, $postData= null){
    $info = array();
 ...
 * 获取后端服务信息
 */
function wpapper_get_info_from_api($url, $param, $checkRes=true){
    $ak_sk = json_decode(get_option(WpApperConf::$option_ak_sk),true);

    $bk = new BkSvr($ak_sk['ak'],$ak_sk['sk']);
    $info = $bk->getInfo($url,$param,false);
    return $info;
}
function wpapper_get_post_info_from_api($url, $param, $checkRes=true){
    $ak_sk = json_decode(get_option(WpApperConf::$option_ak_sk),true);
...
function wpapper_addUrlQueryString($inputUrl, $arrParam = array()){
    if(empty($arrParam)){
        return $inputUrl;
    }
 ...
 */
function wpapper_convert_link2json($src, $type=0){
    $dest = $src;
    switch($type){
        case 1:
            return $dest;
...
function wpapper_check_email($email){
    $status = false;
    if(strlen($email) > 100 ){
        return $status;
    }
 ...
 * 在浏览器展示debug信息
 */
function wpapper_show_debug($msg, $file=__FILE__, $line=0){
 ...
 * 获取插件的url 访问方式
 */
function wpapper_get_plugin_site_base(){
    $webroot = explode("/", WPAPPER_ROOT);
    $plugin_name = $webroot[sizeof($webroot) - 1];
    $site  = plugins_url() . '/'. $plugin_name ;
    return $site;
}
/**
 * 压缩图片
 */
function wpapper_get_resize_img($path, $quality = 10 , $sizes=array()){
...
function wpapper_get_list_pagination($list, $page=1, $limit = 20){

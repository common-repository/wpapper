 ...
 * @return string Full URL to the endpoint.
 */
//返回json的url，如果path为空的话，返回http://wpapper.com:8080/wordpress/wp-json/
function get_json_url( $blog_id = null, $path = '',$action='',$param= array(), $scheme = 'json' ) {
	if ( false && get_option( 'permalink_structure' ) ) {        //如果用户自定义链接,该设置会返回值
		$url = get_home_url( $blog_id, mobileplugin_wpapper::plugin_get_url_prefix(), $scheme );
		...
		$url = add_query_arg( WpApperConf::$app_prefix, 1, $url );
		$url = add_query_arg( WpApperConf::$route_prefix, $path, $url );
		$url = add_query_arg( WpApperConf::$action_prefix, $action, $url );
        if ( !empty($param)){
